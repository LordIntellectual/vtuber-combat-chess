  ...
  public:
    ...
    */
    ...
    /* Adds a piece to the dynamics world and collapse it
      ...
      \param forceScale Multiplier for blast impulse (1 = default, 0 = no kick)
    */
    void collapsePiece(int piece, Vector2i position, float forceScale = 1.f);

    /* Remove all active capture fragments (required before swapping piece sets). */
    void clearFragments();

    /** Rebuild static piece colliders from the chess board (after reset / replay). */
    void resyncBoard(ChessGame* game);
