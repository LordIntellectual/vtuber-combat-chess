...
void SmokeGenerator::draw(Camera* camera){
  ...
  if(nbParticles > 0){
    ...
    // Disable face culling; enable alpha blend for soft sprites
    ...
    // Reset instancing divisors (leave them at 1 and later draws break)
    glVertexAttribDivisor(0, 0);
    ...
    // Critical: smoke binds GL_TEXTURE0..2 and leaves the *active* unit on
    // TEXTURE2. Fixed-function sky then enables/binds on unit 2 while unit 0
    // is empty → blank purple backdrop / untextured sun after piece moves.
    // Also *disable* TEXTURE_2D on each unit so multitex combine doesn't kill
    // the next fixed-function pass.
    for (int u = 2; u >= 0; u--) {
