...
void shadowMappingRender(
  ...
  for(int x = 0; x < 8; x++){
    for(int y = 0; y < 8; y++){
      ...
      // Suggested-move tiles and/or checked king (same bob as main pass)
      ...
      // Draw board cell
      if(meshes->count(BOARDCELL)) meshes->at(BOARDCELL)->draw();

      if(piece != EMPTY) {
        int pk = abs(piece);
        if(meshes->count(pk)) meshes->at(pk)->draw();
      }
    }
  }
